...
// Verifies current authentication status - Used by frontend to check valid session
export const checkAuth = async (req, res) => {
  try {
    const user = await User.findById(req.userId).select("-password"); //! Find user by ID from JWT (excluding password)
    // Handle invalid user (shouldn't happen with valid JWT)
    if (!user) {
      return res
        .status(400)
        .json({ success: false, message: "User not found" });
